...
public class MeetingView {
    ...
    public void exit() {
        io.print("Exiting Meeting Manager");
    }
    ...
    public LocalDateTime updateMeetingTime(LocalDateTime time) {
        int yearInt = time.getYear();
        int monthInt = time.getMonth().getValue();
        int dayInt = time.getDayOfMonth();
        int hourInt = time.getHour();
        int minuteInt = time.getMinute();
        
        String year = io.readString("Enter meeting year(" + yearInt + "): ");
        String month = io.readString("Enter meeting month(" + monthInt + "): ");
        String day = io.readString("Enter meeting day(" + dayInt + "): ");
        String hour = io.readString("Enter meeting hour (0-23) (" + hourInt + "): ");
        String minute = io.readString("Enter meeting minute(" + minuteInt + "): ");
        
        if(!year.isEmpty()) yearInt = Integer.parseInt(year);
        if(!month.isEmpty()) monthInt = Integer.parseInt(month);
        if(!day.isEmpty()) dayInt = Integer.parseInt(day);
        if(!hour.isEmpty()) hourInt = Integer.parseInt(hour);
        if(!minute.isEmpty()) minuteInt = Integer.parseInt(minute);
        
        LocalDateTime newTime = time
                .withYear(yearInt)
                .withMonth(monthInt)
                .withDayOfMonth(dayInt)
                .withHour(hourInt)
                .withMinute(minuteInt);
